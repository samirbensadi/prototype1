...
if (isset($_POST['nom'], $_POST['prenom']) && !empty($_POST['nom']) && !empty($_POST['prenom'])) {
  ...
  if ($result_mail != false) {
    ...
    $req->execute(array('nom' => $nom, 'prenom' => $prenom, 'email' => $email, 'tel' => $tel,'dateNaissance' => $bday, 'mdp_hash' => $mdp, 'formation' => $formation, 'id_statut' => $statut, 'codeQR' => $qrcode, 'confirmation_token' => $confToken)); // exécution de la requete
